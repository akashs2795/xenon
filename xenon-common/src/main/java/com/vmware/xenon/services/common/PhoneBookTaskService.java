...
/**
 * This is an phonebook task service. When a new task is posted, it will delete ALL
 * phonebook services. It does this in two steps: querying for the phonebook services
 * and deleting them.
 *
 * Note that this task service will only be authorized to query or delete phonebook
 * services taht are accessible to the user who created the task.
 *
 * This phonebook task service embodies the typical task workflow, which looks like:
 *
 ...
 *
 */
public class PhoneBookTaskService
        extends TaskService<PhoneBookTaskService.PhoneBookTaskServiceState> {
    ...
    public enum SubStage {
        QUERY_PHONEBOOK, DELETE_PHONEBOOK
    }

    /** Time in seconds for the task to live */
    private static long DEFAULT_TASK_LIFETIME = 60;

    public static final String FACTORY_LINK = ServiceUriPaths.CORE + "/phonebook-tasks";

    /**
     * Create a default factory service that starts instances of this task service on POST.
     */
    public static FactoryService createFactory() {
        return FactoryService.create(PhoneBookTaskService.class, ServiceOption.IDEMPOTENT_POST,
                ServiceOption.INSTRUMENTATION);
    }

    public static class PhoneBookTaskServiceState extends TaskService.TaskServiceState {
        ...
        public QueryTask phoneBookQueryTask;

        /**
         * Optional query clause that will be added, with MUST_OCCUR,
         * to the query used to find phoneBook service instances. The default query
         * uses only the document kind to restrict results to phoneBook service documents.
         */
        @PropertyOptions(usage = {
    ...
    public PhoneBookTaskService() {
        super(PhoneBookTaskServiceState.class);
     ...
     * non-phoneBook tasks will normally have parameters, so this is an phoneBook of how they
     * could be validated.
     */
    protected PhoneBookTaskServiceState validateStartPost(Operation taskOperation) {
        PhoneBookTaskServiceState task = super.validateStartPost(taskOperation);
        if (task == null) {
            return null;
        }
        ...
        if (ServiceHost.isServiceCreate(taskOperation)) {
            ...
            if (task.phoneBookQueryTask != null) {
                taskOperation.fail(
                        new IllegalArgumentException(
                                "Do not specify phoneBookQueryTask: internal use only"));
                return null;
            }
        }
     ...
     * CREATED state.
     */
    protected void initializeState(PhoneBookTaskServiceState task, Operation taskOperation) {
        task.subStage = SubStage.QUERY_PHONEBOOK;
        ...
        // Do our task-specific logic... This will allow our PhoneBookTaskService's "default"
     ...
     * an operation (for phonebook, querying the phonebook services), and when that completes
     * it updates the task state and progresses to the next step by doing a self PATCH.
     *
     */
    @Override
    public void handlePatch(Operation patch) {
        PhoneBookTaskServiceState currentTask = getState(patch);
        PhoneBookTaskServiceState patchBody = getBody(patch);
    ...
    private void handleSubstage(PhoneBookTaskServiceState task) {
        switch (task.subStage) {
        case QUERY_PHONEBOOK:
            handleQueryPhoneBook(task);
            break;
        case DELETE_PHONEBOOK:
            handleDeletePhoneBook(task);
            break;
        default:
     ...
     */
    protected boolean validateTransition(Operation patch, PhoneBookTaskServiceState currentTask,
            PhoneBookTaskServiceState patchBody) {
    ...
    /**
     * Handle SubStage QUERY_PHONEBOOK.
     *
     * Query the query task service for all phonebook services that we are authorized to access.
     * Authorization is implicit: we do not need to indicate it here.
     */
    private void handleQueryPhoneBook(PhoneBookTaskServiceState task) {
        // Create a query for "all documents with kind ==
        // com:vmware:xenon:services:common:PhoneBookService:PhoneBookServiceState"
        Query.Builder builder = Query.Builder.create()
                .addKindFieldClause(PhoneBookServiceState.class);
        ...
        Query phoneBookDocumentQuery = builder.build();
        task.phoneBookQueryTask = QueryTask.Builder.createDirectTask()
                .setQuery(phoneBookDocumentQuery)
                .build();

        // Send the query to the query task service.
        // When we get a response, advance to the next substage, deleting phonebook
        ...
        Operation queryRequest = Operation.createPost(queryTaskUri)
                .setBody(task.phoneBookQueryTask)
                .setCompletion(
                        (op, ex) -> {
                            if (ex != null) {
                            ...
                            // We extract the result of the task because DELETE_PHONEBOOK will use
                            // the list of documents found
                            task.phoneBookQueryTask = op.getBody(QueryTask.class);
                            sendSelfPatch(task, TaskStage.STARTED, subStageSetter(SubStage.DELETE_PHONEBOOK));
                        });
        sendRequest(queryRequest);
    }

    /**
     * Handle SubStage DELETE_PHONEBOOK
     *
     * Delete all of the phonebook service documents that we found in the QUERY_PHONEBOOK substage.
     * We do all the DELETEs in parallel by using OperationJoin.
     */
    private void handleDeletePhoneBook(PhoneBookTaskServiceState task) {
        if (task.phoneBookQueryTask.results == null) {
            sendSelfFailurePatch(task, "Query task service returned null results");
            return;
        }

        if (task.phoneBookQueryTask.results.documentLinks == null) {
            sendSelfFailurePatch(task, "Query task service returned null documentLinks");
            return;
        }
        if (task.phoneBookQueryTask.results.documentLinks.size() == 0) {
            logFine("No phonebook service documents found, nothing to do");
            sendSelfPatch(task, TaskStage.FINISHED, null);
            return;
        }

        List<Operation> deleteOperations = new ArrayList<>();
        for (String phoneBookService : task.phoneBookQueryTask.results.documentLinks) {
            URI phoneBookServiceUri = UriUtils.buildUri(this.getHost(), phoneBookService);
            Operation deleteOp = Operation.createDelete(phoneBookServiceUri);
     ...
     */
    private Consumer<PhoneBookTaskServiceState> subStageSetter(SubStage subStage) {
