 ...
 * Example service
 */
public class PhoneBookService extends StatefulService {

    public static final String FACTORY_LINK = ServiceUriPaths.CORE + "/phonebook";
    ...
    public static FactoryService createFactory() {
        return FactoryService.create(PhoneBookService.class);
    }

    public static class PhoneBookServiceState extends ServiceDocument {
    ...
    public PhoneBookService() {
        super(PhoneBookServiceState.class);
    ...
    public void handleStart(Operation startPost) {
        ...
        // 3) Require that the phoneNumber field is not null
        ...
        PhoneBookServiceState s = startPost.getBody(PhoneBookServiceState.class);
        if (s.name == null) {
            startPost.fail(new IllegalArgumentException("name is required"));
            return;
        }
        if (s.phoneNumber == null) {
            startPost.fail(new IllegalArgumentException("phoneNumber is required"));
            return;
        }
    ...
    public void handlePut(Operation put) {
        PhoneBookServiceState newState = getBody(put);
        PhoneBookServiceState currentState = getState(put);
        ...
        if (currentState.phoneNumber != null && newState.phoneNumber == null) {
            put.fail(new IllegalArgumentException("phoneNumber must be set"));
            return;
        }
    ...
    private PhoneBookServiceState updateState(Operation update) {
        ...
        // below.
        PhoneBookServiceState body = getBody(update);
        PhoneBookServiceState currentState = getState(update);
    ...
    private boolean updateCounter(PhoneBookServiceState body,
                                  PhoneBookServiceState currentState, boolean hasStateChanged) {
    ...
    public void handleDelete(Operation delete) {
        ...
        PhoneBookServiceState currentState = getState(delete);
        PhoneBookServiceState st = delete.getBody(PhoneBookServiceState.class);
    ...
    public ServiceDocument getDocumentTemplate() {
        ...
        PropertyDescription pd = template.documentDescription.propertyDescriptions.get(
                PhoneBookServiceState.FIELD_NAME_KEY_VALUES);
        ...
        PropertyDescription pdName = template.documentDescription.propertyDescriptions.get(
                PhoneBookServiceState.FIELD_NAME_NAME);

        PropertyDescription pdphoneNumber = template.documentDescription.propertyDescriptions.get(
                PhoneBookServiceState.FIELD_NAME_PHONE_NUMBER);

        // instruct the index to enable SORT on this field.
        pdName.indexingOptions.add(PropertyIndexingOption.SORT);
        pdphoneNumber.indexingOptions.add(PropertyIndexingOption.SORT);

        // instruct the index to only keep the most recent N versions
        template.documentDescription.versionRetentionLimit = PhoneBookServiceState.VERSION_RETENTION_LIMIT;
        return template;
    }
}
